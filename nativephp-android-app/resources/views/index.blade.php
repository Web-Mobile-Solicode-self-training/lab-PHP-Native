...
<body class="bg-[#020617] text-slate-200 h-full selection:bg-primary/30" x-data="bookApp(@js($books))"
    :class="{ 'overflow-hidden': isModalOpen || isMenuOpen }">

    <!-- Navigation -->
    <nav
        class="sticky top-0 z-50 glass-effect border-b border-white/5 py-4 px-6 md:px-12 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            <button @click="isMenuOpen = true"
                class="p-2.5 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
            @foreach($books as $index => $book)
                <div class="group relative flex flex-col items-center text-center animate-in fade-in slide-in-from-bottom-4 duration-500"
                    style="animation-delay: {{ $index * 50 }}ms" @click="openInfo(books[{{ $index }}])">

                    <div
                        class="relative w-full aspect-[2/3] rounded-3xl overflow-hidden book-card-shadow transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-primary/20 cursor-pointer">
                    ...
                    <div class="mt-4 w-full px-2">
                        <h3
                            class="font-semibold text-white text-sm line-clamp-1 group-hover:text-primary transition-colors cursor-pointer">
                            {{ $book['volumeInfo']['title'] ?? 'Unknown Title' }}</h3>
                        <p class="text-slate-500 text-xs mt-1 truncate">
                            {{ implode(', ', $book['volumeInfo']['authors'] ?? ['Unknown Author']) }}</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <!-- Modal Drawer -->
    <div x-show="isModalOpen" class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4"
        ...
        <div class="relative w-full max-w-2xl bg-[#0f172a] sm:rounded-[40px] rounded-t-[40px] border-t sm:border border-white/10 overflow-hidden shadow-2xl"
            ...
            x-transition:enter-start="translate-y-full sm:scale-95" x-transition:enter-end="translate-y-0 sm:scale-100">

            <div class="absolute top-6 right-6 z-10">
                <button @click="isModalOpen = false"
                    class="p-2 rounded-full bg-black/40 hover:bg-black/60 transition-colors">
            ...
            <div class="flex flex-col md:flex-row h-full">
                ...
                <div class="flex-1 p-8 md:p-12 overflow-y-auto max-h-[100vw] md:max-h-[600px]">
                    <span
                        class="px-3 py-1 bg-primary/10 text-primary rounded-full text-[10px] font-bold uppercase tracking-widest"
                        x-text="selectedBook?.volumeInfo?.categories?.[0] || 'Book'"></span>
                    <h2 class="text-2xl font-bold text-white mt-4 leading-tight"
                        x-text="selectedBook?.volumeInfo?.title"></h2>
                    ...
                    <div class="mt-8 space-y-4">
                        ...
                        <div class="text-slate-400 text-sm leading-relaxed prose prose-invert"
                            x-html="selectedBook?.volumeInfo?.description || 'No description available for this volume.'">
                        </div>
                    </div>

                    <div class="mt-8 flex gap-3">
                        <a :href="selectedBook?.volumeInfo?.previewLink" target="_blank"
                            class="flex-1 bg-primary text-dark py-4 rounded-2xl font-bold text-center hover:shadow-lg hover:shadow-primary/20 transition-all active:scale-95">Read
                            Preview</a>
                    </div>
                </div>
            </div>
        ...
        <div x-show="isMenuOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <nav class="space-y-6">
                ...
                <template x-for="item in ['Programming', 'Design', 'Business', 'Philosophy', 'Science']">
                    <a :href="'/?q=' + item"
                        class="flex items-center gap-4 text-slate-400 hover:text-white transition-colors group w-full text-left">
                        <span
                            class="w-1.5 h-1.5 rounded-full bg-primary/40 group-hover:bg-primary transition-colors"></span>
